...
if (!defined('MMTY_STL_INTLNKS_VERSION_NUM')){ //define plugin version
    define('MMTY_STL_INTLNKS_VERSION_NUM', '1.4');
}
...
function activate_mmstylishinternallink() {
  add_option('mlstl_background_color');
  add_option('mlstl_border_color');
  add_option('mlstl_textlink_color');
  add_option('mlstl_subheading_color');
...
wp_register_style("stylish-inlinks-icon", plugin_dir_url(__FILE__) . "font-awesome.min.css");
...
function admin_menu_mmstylishinternallink() {
  add_options_page('Stylish Internal Link', 'Stylish Internal Link', 'manage_options', 'stylnk_mskvr_topt', 'options_page_mmstylishinternallink', 'awts_stylink_maker');
}
...
return '<div class="awts-inl"><p>' . $atts['subhd'] . '</p><p><i class="fa fa-link" aria-hidden="true"></i> <a href="' . $atts['link'] . '">'.$content.'</a></p></div>';
}
